...
def test_news():
    data=datasets.fetch_20newsgroups_vectorized(subset='test')
    XX=data.data
    YY=data.target
    X,X_test,target,target_test=train_test_split(XX,YY,test_size=0.5,random_state=33)
    t1 = time.time()
    Y = tsne(X, n=2, max_iter=100, neighbors=200)
    t2 = time.time()
    t = t2 - t1
    print "Custom TSNE cost time: %s" % str(round(t, 2))
    figure1 = pyplot.figure()
    ...
    for i in range(len(data.target_names)):
        xxx1 = Y[target == i, 0]
        xxx2 = Y[target == i, 1]
        pyplot.scatter(xxx1, xxx2, s=10, c=color[i])
        pyplot.text(numpy.median(xxx1),numpy.median(xxx2),
                    data.target_names[i],color='white',
