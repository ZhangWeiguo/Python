...
def train(
    ...
    w_regular     = tf.contrib.layers.l2_regularizer(w_regular_rate)
    ...
    input_vec =  tf.concat([user_vec, item_vec, other_vec], 1)



    regular = input_regular(input_vec)

    for i in range(len(hidden_layers)):
        if i == 0:
            w = tf.Variable(numpy.random.rand(item_dim + user_dim + other_dim, hidden_layers[i]), dtype = tf.float32)
            b = tf.Variable(numpy.random.rand(hidden_layers[i]), dtype = tf.float32)
            mid = tf.nn.relu(numpy.add(tf.matmul(input_vec, w), b))
            regular += w_regular(w)
        else:
            w = tf.Variable(numpy.random.rand(hidden_layers[i-1], hidden_layers[i]), dtype = tf.float32)
            b = tf.Variable(numpy.random.rand(hidden_layers[i]), dtype = tf.float32)
            mid = tf.nn.relu(numpy.add(tf.matmul(mid, w), b))
            regular += w_regular(w)

    w = tf.Variable(numpy.random.rand(hidden_layers[len(hidden_layers)-1], goal_dim), dtype = tf.float32)
    b = tf.Variable(numpy.random.rand(goal_dim), dtype = tf.float32)
    regular += w_regular(w)

    output = tf.sigmoid(tf.add(tf.matmul(mid, w),b), name = "output")
